...
fig3a6 <- 
  ...
  geom_tile(aes(fill = log(SND + plus))) +
  labs(x = "Positive-negative ratio (PNR)", y = "Diagonal dominance (ln)", title = "",
       fill = "Structural niche differences (SND; ln)") +
  scale_x_continuous(expand = c(0.01, 0.01)) +
  scale_y_continuous(expand = c(0.01, 0.01)) +
  scale_fill_viridis_c(option = "A", direction = -1, values = c(-0.1, 1.15)) +
...
ggsave(fig3a, filename = "figures/4metrics_coex_SND.png", device = "png",
       width = 10.5, height = 8, dpi = 320)

ggsave(fig3b_only, filename = "figures/4metrics_coex_SFD.png", device = "png",
       width = 10.5, height = 8, dpi = 320)
